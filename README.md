...
## 시스템 동작 결과 및 실데이터 적용 예시 (2026.03 기준)

실제 최신 재무 데이터를 기반으로 백테스팅을 구동한 핵심 결과는 다음과 같습니다. 

### 1. 포트폴리오 적격성 판정 (Top Picks)
지표 산출 알고리즘은 아래 기업들에 대해 가장 가속화된 성장 효율성(`STRONG_BUY` / `INVEST`)을 포착했습니다.
*   **SK하이닉스 (000660.KS)**: `EGI 2020.6%` (팩터 스코어: 75/100). 추세 기울기 가속, 고강도 효율성 달성 중.
*   **삼성전자 (005930.KS)**: `EGI 222.3%` (팩터 스코어: 80/100). 리스크 요인 0건, 안정적 추세 가속.
*   **Alphabet (GOOGL)**: `EGI 387.7%` (팩터 스코어: 70/100). 강력한 성장 시그널.

### 2. 비효율 / 주의 시그널 감지
*   **NVIDIA (NVDA)**: `EGI 3119.1%`로 절대 효율은 극한에 달했으나, 최근 판관비(31.2% 증가) 및 총 비용(29.4% 증가) 급증으로 인한 변이계수 상승으로 스코어 페널티 적용 (`CAUTION` 판정).
*   **Tesla (TSLA)**: `EGI 176.5%`. 추세 기울기가 급격한 하락세(`sharp_decline`)로 판정되어 리스크 점수 차감.
*   **JPMorgan Chase (JPM)**: 제조/IT 모델과 다른 금융 특성상 `EGI 0.0%`로 산출되어 비적합 자산(`AVOID`)으로 필터링됨.

### 3. 백테스트 시각화 결과
파이프라인 구동 시, 산출된 매매 시그널을 바탕으로 구성된 포트폴리오의 자산 곡선(Equity Curve)과 기업별 EGI 트렌드가 `results/` 디렉토리에 동적 생성됩니다.

![Backtest Results](results/backtest_results.png)
*(위 차트: S&P 500 벤치마크 대비 포트폴리오 누적 수익률 비교)*

![EGI Analysis](results/egi_analysis.png)
*(위 차트: 기업별 EGI 수치 및 임계값 돌파 트렌드 플롯)*
